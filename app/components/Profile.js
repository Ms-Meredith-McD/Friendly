...
function Profile() {
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        const response = await Axios.post(`/profile/${avatar}`, { token: appState.user.token })
  ...
  return (
    <Page title="Profile Screen">
      <h2>
        <img className="avatar-small" src={appState.user.avatar} />{state.profileData.profileUsername}
